/** 必須チェック（空の場合はtrueを返却） */
export function requireCheck(value: string) {
  return !value;
}

/** 記号チェック（記号が含まれている場合はtrueを返却） */
export function symbolCheck(value: string) {
  const reg = RegExp(/[!"¥#$%&'()\*\+\-\.,\/:;<=>?@\[\\\]^_`{|}~]/g);
  return reg.test(value);
}

/** 全角カタカナチェック（カタカナ以外が含まれている場合はtrueを返却） */
export function kanaCheck(value: string) {
...
export function mailaddressCheck(value: string) {
